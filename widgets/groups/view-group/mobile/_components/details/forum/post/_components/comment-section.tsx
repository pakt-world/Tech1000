...
const CommentSection: React.FC<CommentSectionProps> = ({
	...
	return (
		<>
			<motion.div
				className={`${view === "post" ? "mt-1" : ""} w-full`}
					...
					{localComments &&
						...
						localComments.map((comment, index) => (
							<motion.div
								...
								className={` ${view === "post" ? " border-[#FFFFFF33] py-2" : "py-4"} ${localComments?.length !== index + 1 && view !== "trail" ? "border-b border-[#FFFFFF33]" : ""} flex flex-col`}
							...
							>
								...
								>
									...
									<div
										className={`flex w-full flex-col ${view === "post" ? "gap-2" : "gap-2"}`}
									>
										<div className="flex flex-col items-start justify-center">
											...
											<p className="font-mono text-xs capitalize text-[#FFFFFF80]">
												{comment?.creator?.title ||
													"Builder"}
											</p>
										</div>
										<p
											className="font-circular text-[14px] font-normal leading-6 text-[#FFFFFFCC] md:text-[16px]"
										...
										<div
											className={`${view === "post" ? "mt-2" : ""} flex w-full cursor-pointer justify-start gap-2 pb-2 font-bold`}
										>
											<div
												className={`flex items-center gap-1 pr-3  text-xs text-white md:text-sm   ${
													comment?.creator?._id ===
													userState?._id
														? "cursor-text"
														: "cursor-pointer hover:md:rounded-md hover:md:bg-[#CCF9750D]"
												}`}
												onClick={(e) => {
													if (
														comment?.creator
											...
											<div
												className={`flex cursor-pointer items-center gap-1 px-2 text-xs text-white md:text-sm ${
													comment?.creator?._id ===
													userState?._id
														? "cursor-text"
