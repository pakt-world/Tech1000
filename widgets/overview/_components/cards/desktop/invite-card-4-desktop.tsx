...
interface InviteCard4DesktopType {
	feedId: string;
	type:
		| "group_invite"
...
export const InviteCard4Desktop = ({
	feedId,
	type,
	title,
	description,
	...
	applicationId,
	isDismissLoading,
}: InviteCard4DesktopType): JSX.Element => {
	...
	const image = type === "application" ? data?.applicant?.image : data?.image;
	return (
		<FeedWrapper className="h-full !border-[#7DDE86]">
			<SnowProfile src={image} size="lg" />
			<div className="relative flex h-full w-full flex-col justify-between gap-2">
				<div className="flex h-full w-full flex-col items-start  gap-2">
					<div className="flex w-full items-start justify-between">
						...
						{/* {viewType && viewType !== "post_comments" && (
							<X
								size={24}
								className=" right-2 cursor-pointer text-white"
								onClick={() => dismissByID(feedId)}
							/>
						)} */}
					</div>

					<h3 className="whitespace-break-spaces break-words text-lg font-normal text-white">
						{description}
					</h3>
				</div>
						...
						{type === "application" ? (
							<div className="flex gap-6">
								<Button
									size="lg"
									variant="outline"
									className="rounded-full !border-none !p-0 font-bold"
